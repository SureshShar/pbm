...
async function fetchAllRequest() {
  try {
    ...
    showSuccess("Onbording request fetch successfull.");
    if (res.success) {
      list.value = res.data;
    }
  } catch (error) {
    showError("Fail to fetch try some time latter.");
  }
...
async function deleteRequest() {
  try {
    ...
    showSuccess("Onbording request deleted successfully.");
    delete list.value[deleteFileId.value]; // Remove the deleted item from the list 
    showConfirmation.value = false; // Hide the confirmation modal after deletion
    deleteFileId.value = null; // Clear the stored key after deletion
  } catch (error) {
    showError("Fail to delete try some time latter.");
...
function showDeleteConfirmationModel(id) {
  ...
  deleteFileId.value = id; // Store the key of the image to be deleted
}
