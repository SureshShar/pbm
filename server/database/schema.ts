...
export const usersPageData = sqliteTable('users_page_data', {
  ...
  userQuery:     text('user_query', { mode: 'json' }).$type<Record<string, unknown>>(),
  userAssets:    text('user_assets', { mode: 'json' }).$type<Record<string, unknown>>(),
  pageData:      text('page_data', { mode: 'json' }).$type<Record<string, unknown>>(),
